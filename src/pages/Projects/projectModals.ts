...
export const noltoModal = {
  ...
  learnings: [
    ...
    '작은 단위의 컴포넌트부터 bottom-up 방식으로 개발하였으며, 컴포넌트의 유연성과 재사용성을 높이기 위해 고민했습니다. 모든 컴포넌트에 storybook을 작성하여 UI를 확인했습니다.',
    'react-testing-library로 컴포넌트 각 단위 내 기능별 unit 테스트 및 react-hooks-testing-library와 msw를 이용하여 react-query hook 테스트를 작성했습니다.',
    'TypeScript를 사용했습니다. 백엔드의 응답 형식과 프론트 렌더링 형식을 맞춰주기 위해 interface를 확장 가능하도록 고안했으며, 곳곳에서 type guard를 통해 새로운 코드 작성 시 안전하게 타이핑을 할 수 있도록 처리했습니다.',
    '서버와 지속적으로 통신하는 부분이 많은 만큼 react-query를 사용하여 api 요청과 응답의 형식을 잡았습니다. 백엔드와 협의한 error code를 매핑하여 http request에서 에러 발생 시 미리 정해 둔 CustomError 클래스에서 에러 핸들링을 할 수 있도록 했습니다.',
    ...
    '피드 등록/검색 시 기술스택 자동완성을 위해 백엔드 api를 호출하는데, 이때 너무 많은 http request가 발생하지 않도록 debounce를 custom hook으로 구현하여 사용자 input에 따른 api 요청 횟수를 조정했습니다.',
    '성능을 개선하기 위해 번들 파일의 최적화 및 loadable components를 이용한 코드 스플리팅을 구현했습니다. 이미지와 gif 파일 확장자를 변경하여 FCP를 개선했습니다.',
    '컴포넌트의 초기 진입 속도와 SEO를 개선하기 위해 SSR을 적용했습니다. 직접 express 서버를 구축하였으며, nginx를 사용하여 포트 포워딩을 해주었습니다.',
    '코드 스플리팅, 이미지 최적화, SSR 적용을 통해 성능 점수를 73 ➡️ 92점으로 개선하고, 번들 크기를 564kB에서 200kB로 줄였습니다. FCP는 1초대로 개선하였습니다.',
    '안전한 로그인 처리를 위해 refreshToken을 cookie에 저장하고, 접속 중인 클라이언트 IP를 검사하여 express 프론트 서버와 백엔드 서버와의 신뢰도 높은 통신을 구현했습니다.',
